...
'use strict';
    ...
    /**
     * Returns a Regexp.
     * @param q - Search term. It will be searched by this field
     * @param [sep] - It will split q using this separator. Defaults to blank space
     */
    function regexpify (q, sep) {
        if (typeof sep == "undefined" || sep == null) sep = "";

        var reTerm = q;
        var terms = reTerm.split(sep);
        if (terms.length > 1) {
            reTerm = terms[0];
        ...
        return new RegExp(reTerm, "i"); // i = case insensitive
    }

    /**
     * Generates array of conditionals to append to mongoose query.
     * @param params
     */
    module.exports.genQueryAndFilters = function (params) {
        var andFilters = [];

        if (mongoose.Types.ObjectId.isValid(params.branch)) andFilters.push({'branch': params.branch});
        if (mongoose.Types.ObjectId.isValid(params.group)) andFilters.push({'groups': params.group});
        if (mongoose.Types.ObjectId.isValid(params.customer)) andFilters.push({'customer': params.customer});
