(function () {
    ...
    module.exports.find = {
        ...
        byCustomer: function(req, res) {
            ...
            if (typeof req.body.showOnFeed != "undefined"){
                showOnFeed = req.body.showOnFeed;
            }

            eventModel.find({$and: [{customer: req.params.id}, {showOnFeed: showOnFeed}]})
                .sort("-timestamp")
                .populate("house")
                .populate("activity")
                .populate("user", "doc firstName lastName email avatarURL student.character")
                .limit(10)
                .lean()
        ...
        lazyLoad: function(req, res) {
            ...
            eventModel.aggregate( aggregatePipeline ,function(err, result){
                ...
                eventModel.populate(result, {path:'user house achievement activity quest skills newRank item giver', select:'-credentials'}, function(err, result){
                    if (err) errorHandler.handleError(err, req, res);
    ...
    function createEvent(eventData) {
        ...
        newEvent.save()
            .then ( function (newEvent) {
                ...
                newEvent.populate({path:'user house achievement activity quest skills newRank item giver', select:'-credentials'}, function(err){
    ...
    eh.on('questcompleted', function(user, quest) {
        ...
        var _eventData = {
            ...
            showOnFeed: true
            // skills: quest.goals.map(function(g){ return g.skill._id })
            // skills: quest.goals.map(function(g){ return g.skill })
        };
    ...
    eh.on('activitycompleted', function(user, activity) {
        ...
        var _eventData = {
            ...
            showOnFeed: activity.reward.xp > 0
        };
