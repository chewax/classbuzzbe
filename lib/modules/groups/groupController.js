(function () {
    ...
    module.exports.create = function (req, res) {
        ...
        group.save()
            .then (function(result) { return groupModel.populate(result, {path: 'level'})})
        ...
        studentsComplement: function (req, res) {
            ...
            groupModel.findOne({_id:req.params.id})
                .then(function(group){
                    ...
                    var andFilters = [{_id: {$nin: group.students}}];
                    if (mongoose.Types.ObjectId.isValid(group.branch)) andFilters.push({'branch': group.branch});

                    Role.findOne({name:"student"})
                        .then( function(role) {
                            return User.find({roles:role._id})
                                .select("doc firstName lastName avatarURL student.character student.house branch groups")
