(function () {
    ...
    module.exports.update = function (req, res) {
        ...
        houseModel.findOneAndUpdate({_id: element_id}, data, {new: true})
            .then (function (result) { res.status(200).json(result) })
            .catch(function (err) { errorHandler.handleError(err, req, res)});
    };
    ...
    module.exports.find = {
        ...
        members: function(req, res) {
            ...
            return houseModel.find( {customer: req.params.id} )
                    ...
                    userModel.aggregate( query, function ( err, result ) {
                        ...
                        for ( var i = 0; i < houses.length; i++ ) {
                            var idx = _.findIndex( result, function ( h ) {
                                return h._id.equals( houses[i]._id );
                            });
                            ...
                            if ( idx != -1 ) {
                                houseItem.members = result[idx].count;
                            }

                            houseMembers.push(houseItem);
                        }

                        res.status(200).json(houseMembers);
