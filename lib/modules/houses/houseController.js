(function () {
    ...
    function getXPAward(customerId, houseId, activityXpAward) {
        return new Promise(function(resolve, reject){
            getHouseRanking(customerId, houseId)
                .then(function(rank){
                    if (activityXpAward > 0)
                        resolve(Math.round(activityXpAward * config.houses.xpPointsRatio * rank));
                    else
    ...
    eventEmitter.on('activtycompleted', function(user, activity) {
        getXPAward(user.customer, user.student.house, activity.xpAward)
            .then(function(xp){
