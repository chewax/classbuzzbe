(function(){
    ...
    function newProcessRecord(_customer, _branch, _teacher, _group, _detail) {
        ...
        if (typeof  _detail != "undefinded") _proc.detail = _detail;
    ...
    function sendEmailToParents(_st, _final_html, _procId, _subject){
        ...
        return new Promise(function(resolve, reject){
            ...
            if (typeof _st.student.parent.email == "undefined" || _st.student.parent.email == "null" || _st.student.parent.email == "") {
                resolve("success");
                return;
            }
                ...
                .then(function(mRecord){
                    ...
                    if (_st.student.parent2.email != "") {
                        ...
                        var data = {
                            ...
                            subject: '[Class Buzz] - ' + _st.customer.name + ': informe semanal de ' + _st.firstName,
                            html: _final_html
                        };
    ...
    /**
     * Gets users that have 'student' role for a customer & branch & group
     * @param _customer
     * @param _branch
     */
    function getGroupStudents(_customer, _branch, _group) {

        return new Promise(function (resolve, reject) {
            groupModel.findOne({_id:_group})
                .populate('students')
                .then(function(group){
                    resolve(group.students);
                })
                .catch(function (err) {
                    console.log(err);
                    reject(err);
    ...
    function triggerNewMailProcess(_customer, _branch, _teacher, _group, procDetail, iterator){
        ...
        if (typeof _customer === 'undefined' || _customer == null || _customer == "") return triggerProcessForEveryone(procDetail, iterator);
        if (typeof _branch === 'undefined' || _branch == null || _branch == "") return triggerProcessForCustomer(_customer, procDetail, iterator);
        if (typeof _teacher === 'undefined' || _teacher == null || _teacher == "") return triggerProcessForACustomerBranch(_customer, _branch, procDetail, iterator);
        if (typeof _group === 'undefined' || _group == null || _group == "") return triggerProcessForACustomerBranchTeacher(_customer, _branch, _teacher, procDetail, iterator);

        return triggerProcessForACustomerBranchTeacherGroup(_customer, _branch, _teacher, _group, procDetail, iterator);

        //return triggerProcessForACustomerBranch(_customer, _branch, procDetail, iterator);
    }
    ...
    function triggerProcessForACustomerBranchTeacher(_customer, _branch, _teacher, procDetail, iterator) {
        ...
        return new Promise(function(resolve, reject) {
            userModel.findOne({_id: _teacher})
                .then(function (teach) {

                    teach.groups.forEach(function (g) {
                        //If group exists.
                        groupModel.count({_id:g},function(err, c){
                            if (!err && c > 0)
    ...
    function triggerProcessForACustomerBranchTeacherGroup(_customer, _branch, _teacher, _group, procDetail, iterator) {
        ...
        return new Promise(function(resolve, reject){
            newProcessRecord(_customer, _branch, _teacher, _group, procDetail)
                .then(function(proc){
                    _proc = proc;
                    return getGroupStudents(_customer, _branch, _group);
                })
                .then(function(students){
