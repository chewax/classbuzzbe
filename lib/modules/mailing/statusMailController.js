(function () {
    ...
    function createStudentStatusMail(st, _procId, sendMail){
        ...
        var _st = st;
        var _teacher_msg_html, _attendance_html, _achievements_html, _final_html;
        ...
        return renderTeacherMessagesHTML(_st)
            ...
            .then(function(attendance_html){
                ...
                return renderAchievementsHTML(_st);
            })
            .then(function(achievements_html){
                _achievements_html = achievements_html;
                return renderFinalHTML(_st, _teacher_msg_html, _achievements_html, _attendance_html);
            })
    ...
    /**
     * Receives a fully populated student and creates the HTML for the weekly achievements section of the status mail
     * @param _st
     * @returns {Promise}
     */
    function renderAchievementsHTML(_st) {

        return new Promise(function(resolve, reject){

            var lastWeekStarted = new Date().addDays(-7);
            var weekAchievements = _.filter(_st.student.character.achievements, function(a){
            ...
            if (weekAchievements.length > 0) {
                ...
                var alt =  false;
                achievement_html = '<table><thead><tr><th></th><th>Logro</th><th>Recompensa</th></tr></thead><tbody>';

                weekAchievements.forEach(function(a){

                    if (!alt) {
                        achievement_html += '<tr class="'+ a.achievement.type +'">';
                    }
                    else achievement_html += '<tr class="alt '+ a.achievement.type +'">';

                    if (a.achievement.type == 'reward'){
                        achievement_html += '<td class="content-block aligncenter icon">' +
                    ...
                    if (typeof a.achievement.nameTranslation != 'undefined')
                        achievement_html += a.achievement.name + '/' + a.achievement.nameTranslation;
                    else
                        achievement_html += a.achievement.name;

                    achievement_html += '</td><td>';
                    achievement_html += a.achievement.xpAward +'xp';
                    achievement_html += '</td></tr>';
                    alt = !alt;
                });

                achievement_html += '</tbody></table>';
            }

            else {

                achievement_html += '<table><thead><tr><th>No recibi&oacute; ning&uacute;n m&eacute;rito esta semana</th></tr></thead><tbody>';
            }


            resolve(achievement_html);
        });
     ...
     * @param _st
     * @param _mails_html
     * @param _achievements_html
     * @param _attendance_html
     * @returns {*}
     */
    function renderFinalHTML(_st, _mails_html, _achievements_html, _attendance_html){
        ...
        return new Promise(function(resolve, reject){
            var templateData = {
                ...
                weekly_achievements_html: _achievements_html,
