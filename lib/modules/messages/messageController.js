(function () {
    ...
    /**
     * Creates a notification for the achievement award
     * and broadcast the event so that socketIO controller can push the notification
     */
    eh.on('achievementaward', function(user, achievement) {

        var newMessage = new messageModel();
        newMessage.title = 'Achievement Unlocked: ' + achievement.name;
        newMessage.type = "achievementUnlocked";
        newMessage.message = "You have unlocked a new achievement and earned " + achievement.xpAward + 'XP.';

        if (achievement.type == 'penalty') {
            newMessage.title = 'Penalization: ' + achievement.name;
            newMessage.type = "penaltyAchievement";
            newMessage.message = "You have received a penalization and have forfeited " + achievement.xpAward + 'XP.';
        }
