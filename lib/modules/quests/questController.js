(function () {
    ...
    eh.on('achievementaward', function(user, achievement) {
        questModel.find({'trigger.achievement': achievement._id})
            .then(function(quests){

                if (quests.length <= 0) return; // No quests associated to that achievement

                //Else check if user has any of that quest associated to him/her.
                quests.forEach( function (achievementQuest) {
                    var index = user.student.quests.indexOf(achievementQuest._id);
                    if (index > -1) { //the user has the quest.
                        applyQuestReward(user, achievementQuest)
                            .then( function(result) {
                                eh.emit('questcompleted', user, achievementQuest);                  //Bradcast "questcompleted" event
