(function () {
    ...
    module.exports.findByName = function(req, res) {
        ...
        var role = roleModel.findOne({name: roleName}, function (err, result){

            if (err) {
                logger.error(messages.log.error("Error while retrieving role: " + roleName, req, err));
                res.status(500).send(messages.error.onFetch('role'));
                return;
            }

            res.status(200).send(messages.success.onAction("Fetch user"));
