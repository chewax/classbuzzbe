(function(){
    ...
    PlaySession.prototype.resume = function(userId) {
        ...
        if (this.status != sUtils.status.PAUSED || this.status != sUtils.status.PULSED) return;
     ...
     * @param {function} cb - Callback when countdown ends
     * @emits session.game.start
     *
     * Calls _onCountdownTick function. Intentded to be overriden by custom behaviour on coundown.
     * @private
     */
    PlaySession.prototype._startCountdown = function (count, tick, cb) {

        if (typeof cb != 'function') cb = this._onCountdownFinish;

        this.status = sUtils.status.COUNTDOWN;
        var that = this;

        function _tick() {
            that._onCountdownTick(count);
