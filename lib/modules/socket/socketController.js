(function () {
    ...
    eh.on('session.status.change', function(_session, status) {
        //There should be a channel for session _id.
        io.to(_session._id).emit('session.status.change', _session, status);
    });
    ...
    eh.on('session.countdown.tick', function(_session, count) {
        io.to(_session._id).emit('session.countdown.tick', _session, count);
    });

    eh.on('session.game.start', function(_session) {
        io.to(_session._id).emit('session.game.start', _session);
    });

    eh.on('session.game.paused', function(_session) {
        io.to(_session._id).emit('session.game.paused', _session);
    });

    eh.on('session.game.resumed', function(_session) {
        io.to(_session._id).emit('session.game.resumed', _session);
    });

    eh.on('session.unable.start', function(_session, reason) {
        io.to(_session._id).emit('session.unable.start', _session, reason);
    });

    eh.on('session.game.pulse', function(_session, userId) {
        //Meaning that userId has pulsed the pulser...and is the first also.
        io.to(_session._id).emit('session.game.pulse', _session, userId);
    });
