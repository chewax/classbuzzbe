(function () {
    ...
    module.exports.teacher = {
        ...
        activityYearly: function(req, res) {
            teacherActivityYearly(req.params.id, req.body.year)
                .then(function (result) {
                    res.status(200).json(result);
                })
                .catch(function (err) {
    ...
    function teacherActivityYearly(teacherId, year) {
        var from;

        if (typeof year != 'undefined') from = moment(year.toString()+'-01-01').startOf('year');
        else from = moment().startOf('year');
        ...
        return Events.find({giver: teacherId})
            .and([{timestamp: {$gte: from}}, {activity: {$ne:null}}])
            ...
            .then(function (events) {
                ...
                events.forEach(function(e){
                    ...
                    byMonth[month].activities.push(e);

                    if (e.activity.reward.xp >= 0) byMonth[month].positive += 1;
                    if (e.activity.reward.xp < 0) byMonth[month].negative += 1;

                    e.activity.skills.forEach(function(s){
                        if (typeof byMonth[month].skills[s.name] == 'undefined') byMonth[month].skills[s.name] = 1;
                        else byMonth[month].skills[s.name] += 1;
                    })
                })
