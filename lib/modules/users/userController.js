(function () {
    ...
    eventEmitter.on('levelup', function(user, xp) {
        ...
        eventController.create(_eventData);
    });
    ...
    module.exports.activity = {
        ...
        add: function (req, res) {
            ...
            var _activity = req.body._id;

            User.findOne( {_id: req.params.id} )
                .then(function (user){
                    user.student.character.activities.push({activity:_activity});
                    return user.save();
                })
                .then(function (user){
                    //Emit activityCompleted event
                    eventEmitter.emit('activitycompleted', user, _activity);
                    res.status(200).json(user);
                })
                .catch(function (err) { errorHandler.handleError(err, req, res) });
        }
