(function () {
    ...
    var Email = require('../emails/emailModel');
    ...
    module.exports.find = {
        ...
        publicProfile: function (req, res) {
            User.findOne( {_id: req.params.id}, '-__v -credentials -activity')
                .populate('groups student.house branch customer roles')
                .then(function (results){ res.status(200).json(results);})
                .catch(function (err) { errorHandler.handleError(err, req, res) });
        },
    ...
    module.exports.quests = {
        ...
        all: function(req, res){
            User.findOne({_id: req.params.id})
                .populate('student.character.quests.goals.skill student.character.quests.quest')
                .then( function (user){res.status(200).json(user.student.character.quests); })
                .catch(function (err) { errorHandler.handleError(err, req, res) })
        }
    };
    ...
    module.exports.lookForNewQuests = function(userId){
        ...
        console.info ("Looking for new quests for user: ", userId);
        var _user;
            ...
            .then( function (user) {
                ...
                Quest.find({customer: user.customer})
                    .then(function(quests){
                        ...
                        var userQuests = _user.student.character.quests.map(function(q){
                            if (q.completed == null)
