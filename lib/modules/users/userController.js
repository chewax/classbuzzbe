(function () {
    ...
    module.exports.adHocParentUpdateMailingProcess = function(req, res) {
        Role.findOne({name:'student'})
        .then(function(role){
            return User.find({roles:role._id}).populate('customer branches student.character.activities.activity student.character.skills.skill');
        })
        .then(function(students){
            res.status(200).send("Newsletter process started!");

            students.forEach(function(st){
                statusMailsController.createStudentStatusMail(st, null, true, new Date(req.query.fromDate), new Date(req.query.toDate) );
            })
        })
        .catch( function(err){ errorHandler.handleError(err, req, res); })
    }
