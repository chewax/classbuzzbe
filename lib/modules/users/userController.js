(function () {
        ...
        add: function (req, res) {
            ...
            User.findOne( {_id: req.params.id} )
                ...
                .then(function (user){
                    ...
                    return Activity.findOne({_id: req.body._id});
                })
                .then(function(_activity){
                    //Emit activityCompleted event
                    eventEmitter.emit('activitycompleted', _user, _activity);
                    res.status(200).json(user);
                })
                .catch(function (err) { errorHandler.handleError(err, req, res) });
        }
    ...
    module.exports.quests = {
        ...
        all: function(req, res){
            User.findOne({_id: req.params.id})
                ...
                .deepPopulate('student.character.quests.goals.goal.skills')
            ...
            .then( function (user) {
                ...
                Quest.find({customer: user.customer})
                    ...
                    .then(function(quests){
                        ...
                        //TODO Refactor this to use virtualField activeQuests.
    ...
    eventEmitter.on('activitycompleted', function(user, activity) {
        recalculateGoalsSteps(user, activty);
        checkForCompletedGoals (user);
    });

    eventEmitter.on('goalcompleted', function(user, goal) {
        checkForCompletedQuests (user);
    });

    eventEmitter.on('questcompleted', function(user, quest) {
        applyQuestRewards (user, quest);
    });
    ...
    /**
     * For every user goal, checks if goal is fulfiled
     * @param user
     * @param activity
     * @emits 'goalcompleted' event
     */
    function checkForCompletedGoals(user){
        //TODO Finish this
        //TODO Tip: use virtual attribute isFulfiled
        //If any goal is completed with this acitivty emitEvent
        var _goal = {
            _id: "notAMongooseValidId",
            name:"Dummy Goal"
        };

        eventEmitter.emit('goalcompleted', user, _goal);
        return;
    }

    /**
     * For every user quest checks if quest isCompleted
     * @emits 'questcompleted' event
     * @param user
     */
    function checkForCompletedQuests (user){
        //TODO Finish this
        //TODO Tip: use virtual attribute isCompleted
        //If any goal is completed with this activity emitEvent
        var _quest = {
            _id: "notAMongooseValidId",
            name:"Dummy Quest"
        };
        eventEmitter.emit('questcompleted', user, _quest);
        return;
    }

    /**
     * Applies quest rewards to user
     * @param user
     * @param quest
     */
    function applyQuestRewards (user, quest){
        //TODO Finish this
        return;
    }
