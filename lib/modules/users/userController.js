(function () {
        ...
        add: function (req, res) {
            ...
            User.findOne( {_id: req.params.id} )
                ...
                .then(function(_activity) {
                    ...
                    //I am not saving...next step (one of the functions suscribed to the event will save) which will check for completed quests will save.
    ...
    module.exports.quests = {
        ...
        all: function(req, res){
            User.findOne({_id: req.params.id})
                ...
                .then( function (user){
                    var _active = _.filter(user.student.character.quests, 'completed',  null);
                    res.status(200).json(_active);
                })
                .catch(function (err) { errorHandler.handleError(err, req, res) })
        }
    };
        ...
        buy: function(req, res) {
            ...
            User.findOne({doc: req.params.doc})
                ...
                .then(function(result){
                    ...
                    eh.emit('newnotification', _user, newMessage);
    ...
    function recalculateGoalsSteps(user, activity){
        ...
        return User.populate(user, {path:'student.character.quests.goals.goal student.character.quests.quest'})
            .then(function(_user){
                ...
                _user.student.character.activeQuests.forEach(function(q){
                    ...
                    q.goals.forEach(function(g){
                        ...
                        //If goal is already fulfiled then return...
                        if (g.isFulfiled) return;

                        //If goal matches with activity increment completed steps
                        if (g.goal.matchesActivity(activity)) g.completedSteps += 1;

                        //If is fulfiled, emit event
                        if (g.isFulfiled) eh.emit('goalcompleted', _user, g.goal);
                    ...
                    if (q.isCompleted)  {
                        ...
                        //Apply Rewards;
                        _user.student.character.xp += q.quest.reward.xp;
                        _user.student.character.money += q.quest.reward.money;
                    }
