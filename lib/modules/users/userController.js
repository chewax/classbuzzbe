(function () {
    ...
    /** EVENTS */
    eventEmitter.on('levelup', function(user, xp) {
        var level = utils.getUserLevel(user.student.character.xp + xp);
        var dsc = user.firstName+' '+user.lastName+' has reached level ' + level;
        eventController.create( user._id, "Level UP", dsc, xp);
    });

    eventEmitter.on('levellost', function(user, xp){
        var dsc = user.firstName+' '+user.lastName+' has lost a level!';
        eventController.create( user._id, "Level Lost", dsc, xp);
    });

    eventEmitter.on('xpaward', function(user, xp) {
        var dsc = user.firstName+' '+ user.lastName+' has been awarded '+ xp + ' experience points.';
        eventController.create(user._id, "XP Award", dsc, xp);

        if (utils.willLevelUp(user.student.character.xp, xp)) {
            eventEmitter.emit('levelup', user, xp);
        }
    });

    eventEmitter.on('xpforfeit', function(user, xp){
        var dsc = user.firstName+' '+user.lastName+' has forfeited '+ xp + ' experience points.';
        eventController.create(user._id, "XP Forfeit", dsc, xp);

        if (utils.willLevelDown(user.student.character.xp, xp)) {
            eventEmitter.emit('levellost', user, xp);
        }
    });
    /** END EVENTS */
    ...
    module.exports.updateAvatarURL = function (req, res) {
        ...
        User.findOneAndUpdate( {_id: req.body._id}, {avatarURL:req.body.avatarURL}, {upsert:false})
            .then( function (results){ utils.handleSuccess(messages.success.onAction("updating user"), res);})
    ...
    module.exports.update = function (req, res) {
        ...
        User.findOneAndUpdate( {_id: userid}, data, {upsert:false})
            .then( function (results){ utils.handleSuccess(messages.success.onAction("updating user"), res);})
            .catch(function (err) { errorHandler.handleError(err, req, res) });
    };
    ...
    module.exports.newStudent = function (req, res) {
        ...
        utils.hashPasswordAsync(req.body.credentials.password)
            ...
            .then( function (renderedHTML) {
                ...
                utils.handleSuccess(messages.success.onCreate("Student"), res);
    ...
    module.exports.status = {
        set: function(req, res) {
            User.findOneAndUpdate( {doc: req.body.doc}, {isActive:req.body.isActive}, {upsert:false})
                .then( function (results){ utils.handleSuccess(messages.success.onAction("updating user"), res);})
                .catch(function (err) { errorHandler.handleError(err, req, res) });
        },

        get: function(req, res) {
            User.findOne( {doc: req.body.doc})
                .select('isActive')
    ...
    module.exports.newUser = function (req, res) {
        ...
        utils.hashPasswordAsync(req.body.credentials.password)
            ...
            //.then (function (result) { utils.handleSuccess(messages.success.onCreate("User"), res) })
    ...
    module.exports.remove = function (req, res) {
        ...
        User.findOne({doc: req.body.doc})
            ...
            .then (function (result) {
                utils.handleSuccess(messages.success.onDelete("User"), res)
            })
            .catch(function (err) {
                errorHandler.handleError(err, req, res)
            });
    ...
    module.exports.student = {
        ...
        findTeachers: function(req, res) {
            User.findStudentTeachers(req.params.doc)
        ...
        findByTeacher: function(req, res) {
            ...
            //Find the teacher
            User.findOne({doc:req.params.doc})
                .then(function(teacher){
                   //Find teachers groups
                   return Group.find({_id: {$in:teacher.groups}})
                })
                .then(function(groups){
                    ...
                    return User.find({_id:{$in:_students}})
                        .populate("groups student.house")
                        .select("doc firstName lastName email avatarURL student.parent student.character.xp groups student.house")
                        .lean(true);
                })
            ...
            User.findOne({doc:req.params.doc})
                ...
                .then(function(groups){
                    ...
                    var pagOptions = {
                        ...
                        columns: 'doc firstName lastName email avatarURL student.parent student.character.xp groups house',
                        populate: 'roles groups student.house',
                        lean: true
                    };

                    return User.find({_id:{$in:_students}})
                        .populate("groups house")
                        .select("doc firstName lastName email avatarURL student.parent student.character.xp groups house")
                        .lean(true);
                })
                .then(function(_students){
    ...
    module.exports.find = {
        ...
        one: function (req, res) {
            User.findOne( {doc: req.params.doc}, '-__v -credentials -activity')
                .populate('groups student.house branches')
                .deepPopulate('roles.mayGrant parent.children')
                .lean(true)
                .then(function (user){
                    ...
                    var levelInfo =  utils.getLevelInfo(user.student.character.xp);
                    user.student.character.levelInfo = levelInfo;
        ...
        publicProfile: function (req, res) {
            User.findOne( {_id: req.params.id}, '-__v -credentials -activity')
                .populate('groups student.house branches customer roles')
                .then(function (results){ res.status(200).json(results) ;})
                .catch(function (err) { errorHandler.handleError(err, req, res) });
        },
        ...
        studentNotInGroup: function (req, res) {
            ...
            var andFilters = [{branches: {$in: req.body.branches}}, {groups: {$nin:[req.body.group]}}];
        ...
        paginate: function (req, res) {
            ...
            if (mongoose.Types.ObjectId.isValid(req.body.searchFilters.branch)) andFilters.push({'branches': req.body.searchFilters.branch});
            if (mongoose.Types.ObjectId.isValid(req.body.searchFilters.group)) andFilters.push({'groups': req.body.searchFilters.group});
            ...
            if (andFilters.length > 0) {
                ...
                var pagOptions = {
                    page: req.body.page || 1,
                    limit: req.body.limit || 10,
                    columns: 'doc firstName lastName email phoneNumber avatarURL roles groups',
                    populate: 'roles',
                    lean: true
                };
         ...
         * @param  {response}
         */
        popGroup: function (req, res) {
            User.findOne( {doc: req.params.doc}, '-__v -credentials -activity')
                .deepPopulate('roles.mayGrant parent.children groups.students groups.level')
                .then(function (user){
    ...
    module.exports.role = {
        ...
        add: function(req, res) {
            User.findOne ({doc: req.params.doc})
                ...
                .then (function (result) { utils.handleSuccess(messages.success.onAction("adding role"), res) })
                .catch(function (err) { errorHandler.handleError(err, req, res)});
        },
        ...
        remove: function (req, res) {
            ...
            User.findOne({doc: req.params.doc})
                ...
                .then (function (result) { utils.handleSuccess(messages.success.onAction("deleting role"), res) })
                .catch(function (err) { errorHandler.handleError(err, req, res)});
    ...
    module.exports.children = {
        ...
        add: function(req, res) {
                    ...
                    User.find({doc: {$in: docsToAdd }}, '_id').then(
                        ...
                        function (result) {
                            ...
                            if (utils.isEmpty(result)) {
                                ...
                                var newOnes = utils.arrayDifference(idsToAdd, user.parent.children);
            ...
            User.findOne({doc: req.params.doc})
                ...
                .then (function (result) { utils.handleSuccess(messages.success.onAction("Add children"), res) })
                .catch(function (err) { errorHandler.handleError(err, req, res)});
        },
        ...
        remove: function(req, res) {
            ...
            User.findOne({doc: req.params.doc})
                ...
                .then (function (result) { utils.handleSuccess(messages.success.onAction("Remove children"), res) })
                .catch(function (err) { errorHandler.handleError(err, req, res)});
        },

        /**
         * Updates a users children parental control
         * Body must have updated parental control record
         * @param req
         * @param res
         */
        updateParentalControl: function(req, res){
            User.findOne({doc: req.params.doc})
                .populate ('roles')
                .then (function (user) { return authMid.userHasRoles(user, ['parent']); })
                .then (function (user) { return User.findOne({doc: req.params.child_doc});})
                .then (function (child) {
                    child.student.parentalControl = req.body;
                    return child.save();
                })
                .then (function (result) { utils.handleSuccess(messages.success.onAction("update child parental control"), res) })
                .catch(function (err) { errorHandler.handleError(err, req, res)});
        },

        getParentalControl: function(req, res){
            User.findOne({doc: req.params.doc})
                .populate ('roles parent.children')
                .then (function (user) { return authMid.userHasRoles(user, ['parent']); })
                .then (function (user) {
                    var userChildren = user.parent.children.map( function(c){ return c.doc; } );
                    var index = userChildren.indexOf(req.params.child_doc);
                    res.status(200).json(user.parent.children[index].student.parentalControl);
                })
                .catch(function (err) { errorHandler.handleError(err, req, res)});
        }
    ...
    /**
     * Achievements Handling Functions
     * @type {{add: Function, remove: Function, all: Function}}
     */
    module.exports.achievement = {
        add : function(req, res) {
            ...
            // Find and working user
            User.findOne({doc: req.params.doc})
                .then (function (user) {
                    // Keep working user and find and working achievement
                    _user = user;
                    return Achievement.findOne({_id: req.body.achievementId});
                })
                .then (function (achievement) {
                    // Keep working achievement and add achievement to user
                    _achievement = achievement;
                    return _user.addAchievement(achievement);
                })
                .then (function (results){
                    // Create Associated event
                    _results = results;
                    eventEmitter.emit('achievementaward', _user, _achievement);

                    if (_achievement.type == 'reward') {
                        var messageText = _user.firstName + ' ' + _user.lastName + ' has earned a "' + _achievement.name + '" Award.';

                        //New requiremente, grant some gold whenever you receive an achievement.
                        _user.student.currency.doubloons += Math.round(_achievement.xpAward * 0.3 / 5) * 5;
                        _user.save();
                        return eventController.create(_user._id, 'Achievement Unlocked', messageText, _achievement.xpAward, _achievement._id, req.user.id);
                    }

                    if (_achievement.type == 'penalty') {
                        var messageText = _user.firstName + ' ' + _user.lastName + ' has received a "' + _achievement.name + '" Penalty.';
                        return eventController.create(_user._id, 'Penalty', messageText, _achievement.xpAward, _achievement._id, req.user.id);
                    }

                })
                .then (function (results){
                // Return new character data
                res.status(200).json(_results.student.character);
            })
                .catch(function (err) { errorHandler.handleError(err, req, res) });
        },
        ...
        remove: function(req, res) {
            ...
            Event.findOneAndRemove({achievement: req.body.achievementId})
                .then( function (result) { return User.findOne( {doc: req.params.doc}) })
                .then (function (user) {
                    eventEmitter.emit('achievementforfeit', _user, _achievement);
                    return user.removeAchievement (req.body.achievementId);
                })
                .then (function (results){ res.status(200).json(results.student.character) ;})
                .catch(function (err) { errorHandler.handleError(err, req, res) });
        },

        all: function (req, res) {
            User.findOne( {doc: req.params.doc}, "student.character.achievements")
                .populate('student.character.achievements.achievement')
                .then(function (results){ res.status(200).json(results.student.character.achievements) ;})
                .catch(function (err) { errorHandler.handleError(err, req, res) });
        },
        ...
        find: function (req, res) {
            User.findOne( {doc: req.params.doc}, "student.character.achievements")
                .populate('student.character.achievements.achievement')
                .then(function (results){
                    var achievements = results.student.character.achievements;
                    var sortedAchievements = _.sortBy(achievements, function(a){
                        return a.timestamp;
                    })
                    var toReturn = _.last(sortedAchievements, req.body.limit);
                    res.status(200).json(toReturn.reverse());
                })
                .catch(function (err) { errorHandler.handleError(err, req, res) });
        }
    };
    ...
    module.exports.character = {
        ...
        /**
         * Add XP to a user character
         * @param req
         * @param res
         */
        addXP: function (req, res) {
            var _user;

            User.findOne( {doc: req.params.doc})
                .then (function (user) {
                    //Find and update user XP
                    _user = JSON.parse(JSON.stringify(user)); //To clone user.
                    var xp = req.body.xp;

                    eventEmitter.emit('xpaward', _user, parseInt(xp));
                    user.student.character.xp += parseInt(xp);

                    return user.save();
                })
                .then (function (results){
                    var resObj = utils.getLevelInfo(results.student.character.xp);
        ...
        /**
         * Del XP from a user character
         * @param req
         * @param res
         */
        delXP: function (req, res) {
            var _user;
            ...
            User.findOne({doc: req.params.doc})
                .then(function (user) {
                    ...
                    eventEmitter.emit('xpforfeit', _user, xp);
                    user.student.character.xp -= xp;
                    return user.save();
                })
                .then(function (results) {
                    var resObj = utils.getLevelInfo(results.student.character.xp);
        ...
        /**
         * Add a character to user passed by parameter in the API endpoint
         * @param req
         * @param res
         */
        create: function (req, res) {
            User.findOneAndUpdate( {doc: req.params.doc}, { "student.character": req.body}, {upsert:false})
                .then (function (result) { utils.handleSuccess(messages.success.onCreate("Character"), res) })
                .catch(function (err) { errorHandler.handleError(err, req, res)});
        },
        ...
        update: function (req, res) {
            User.findOneAndUpdate( {doc: req.params.doc}, {"student.character": req.body}, {upsert:false})
                .then( function (results){
                    utils.handleSuccess(messages.success.onAction("updating character"), res);})
                .catch(function (err) { errorHandler.handleError(err, req, res) });
        }
    };

    /**
     * Groups Handing Functions
     * @type {{all: Function, add: Function, remove: Function}}
     */
    module.exports.groups = {
        all: function(req, res){
            User.findOne( {doc: req.params.doc} )
                .select('groups')
                .deepPopulate('groups.level')
                .then(function (results){ res.status(200).json(results.groups);})
                .catch(function (err) { errorHandler.handleError(err, req, res) });
        },

        /**
         * CREATES GROUP AND ADDS TO OWNER...NOT TO CREATE STUDENTS JUST OWNER.
         * @param req
         * @param res
         */
        createAndAdd: function (req, res) {
            ...
            newGroup.save()
                ...
                .then (function (results){ res.status(200).json(newGroup) ;})
                .catch(function (err) { errorHandler.handleError(err, req, res) });
        },

        //DONT USE TO LINK TO GROUP OWNER
        add: function (req, res) {
            User.findOne( {doc: req.params.doc})
                .then (
                function (user) {

                    var index = user.groups.indexOf(req.body._id);
                    if (index == -1) user.groups.push(req.body._id);
                    Group.addUser(req.body._id, user._id);
                    return user.save();
        ...
        remove: function (req, res) {
            User.findOne( {doc: req.params.doc})
                ...
                function (user) {
                    ...
                    Group.removeUser(req.body._id, user._id);
                    return user.save();
                })
                .then (function (results){ res.status(200).json(results.groups) ;})
                .catch(function (err) { errorHandler.handleError(err, req, res) });
    ...
    /**
     * Branches handling Functions
     * @type {{all: Function, update: Function, add: Function, remove: Function}}
     */
    module.exports.branches = {

        /**
         * Returns all User Branches
         * @param req
         * @param res
         */
        all: function (req, res) {
            User.find( {doc: req.params.doc} )
                .select('branches')
                .deepPopulate('branches.headmaster')
                .then(function (results){ res.status(200).json(results.branches) ;})
                .catch(function (err) { errorHandler.handleError(err, req, res) });
        },

        /**
         * Updates a user branch. Body must have new branch id or new isActive state.
         * If either of them is missing it will leave the property untouched.
         * @param req
         * @param res
         */
        update: function (req, res) {
            User.findOne( {doc: req.params.doc})
                .then (function (user) {
                user.branch = req.body.id || user.branch;
                user.isActive = req.body.isActive || user.isActive;
                return user.save();
            })
                .then( function (results){ utils.handleSuccess(messages.success.onAction("deleting achievement"), res);})
                .catch(function (err) { errorHandler.handleError(err, req, res) });
        },

        /**
         * Adds a branch to the branch list of the user
         * @param req
         * @param res
         */
        add: function (req, res) {
            User.findOne( {doc: req.params.doc})
                .then( function (user) {
                    user.branches.push(req.body.branchId);
                    return user.save();
                })
                .then( function (results){ utils.handleSuccess(messages.success.onAction("Add Branch To User"), res);})
                .catch(function (err) { errorHandler.handleError(err, req, res) });
        },

        /**
         * Removes a branch from the branch list of the user
         * @param req
         * @param res
         */
        remove: function (req, res) {
            User.findOne( {doc: req.params.doc})
                .then( function (user) {
                    var index = user.branches.indexOf(req.body.branchId);
                    if (index != -1) user.branches.splice(index, 1);
                    return user.save();
                })
                .then( function (results){ utils.handleSuccess(messages.success.onAction("Remove Branch from User"), res);})
                .catch(function (err) { errorHandler.handleError(err, req, res) });
        }
    };
    ...
    module.exports.house = {
        ...
        update: function (req, res) {
            User.findOne( {doc: req.params.doc})
                ...
                .then( function (results){ utils.handleSuccess(messages.success.onAction("updating house"), res);})
                .catch(function (err) { errorHandler.handleError(err, req, res) });
        }
    };
    ...
    module.exports.activity = {
         ...
         */
        all: function (req, res) {
            User.findOne( {doc: req.params.doc}, 'activity')
                .then(function (results){ res.status(200).json(results.activity) ;})
                .catch(function (err) { errorHandler.handleError(err, req, res) });
        },

        /**
         * Allows query a users activity. Assumes body has query
         * @param  {request}
         * @param  {response}
         */
        query: function (req, res) {
            req.body.doc = req.params.doc;
            User.findOne(req.body, 'activity')
                .then(function (results){ res.status(200).json(results.activity) ;})
                .catch(function (err) { errorHandler.handleError(err, req, res) });
        }
    };
    ...
    module.exports.quests = {
        add: function(req, res){
            User.addQuest(req.params.doc, req.body.questId)
                .then( function (results){ res.status(200).json(results); })
                .catch(function (err) { errorHandler.handleError(err, req, res) });
        },

        remove: function(req, res){
            User.removeQuest(req.params.doc, req.body.questId)
                .then( function (results){ res.status(200).json(results); })
                .catch(function (err) { errorHandler.handleError(err, req, res) });
        },
        ...
        all: function(req, res){
            User.findOne({doc: req.params.doc})
                .populate('student.quests')
                .then( function (user){ res.status(200).json(user.student.quests); })
                .catch(function (err) { errorHandler.handleError(err, req, res) })
        },

        allWithAchievement: function(req, res){
            User.findOne({doc: req.params.doc})
                .deepPopulate('student.quests.trigger.achievement')
                .then( function (user){ res.status(200).json(user.student.quests); })
                .catch(function (err) { errorHandler.handleError(err, req, res) })
        },
    };
        ...
        buy: function(req, res) {
            ...
            User.findOne({doc: req.params.doc})
                ...
                .then( function (trait){
                    ...
                    if (_user.student.currency.doubloons < _trait.price) throw new customErr.notEnoughMoney();

                    // Decrement money available.
                    _user.student.currency.doubloons -= _trait.price;
                ...
                .then(function(result){
                    ...
                    // Log Action
                    logger.info("User: " + _user._id +', has purchased trait: ' + _trait._id);

                    // Emit message
                    var newMessage = new Message();
                    newMessage.user = _user._id;
                    newMessage.title = "Trait Acquired!";
                    newMessage.message = 'You have successfully purchased ' + _trait.name;
                    newMessage.type = "newTrait";
                    newMessage.save();

                    eventEmitter.emit('newnotification', _user, newMessage);

                    //Send Result
                    var _resObj = {
                        status:200,
                        message: "Purchase has been successfully completed",
                        currency: _user.student.currency,
    ...
    module.exports.parent = {
        ...
        new: function (req, res) {
            ...
            User.findOne({$or: [{'student.parent.doc': parent_user_doc}, {'student.parent2.doc': parent_user_doc}]})
                ...
                .then(function (result) { utils.handleSuccess(messages.success.onCreate("Parent"), res) })
                .catch(function (err) { errorHandler.handleError(err, req, res) });
    ...
    module.exports.lookForNewQuests = function(userId){
        User.findOne({_id: userId})
            ...
            .then( function (user) {
                ...
                var studentRole = _.findWhere(user.roles, {name:"student"});
                if (_.isEmpty(studentRole)) return; //It is not student, do not add quest.

                if (user.student.lastQuestAssignment != null){
                    //has quest assigned already, lets check the date
                    var nextQuestDate = user.student.lastQuestAssignment.add({days: config.quests.respawnTime});
                    if (Date.now() < nextQuestDate || user.student.quests.length >= 3) {
                ...
                Quest.find({customer: user.customer})
                    .then(function(quests){
                        ...
                        var userQuests = user.student.quests.map(function(q){return q.toString()});

                        var assignableQuests = _.difference(questIdList, userQuests); //Get the ones that the user isnt assigned yet
                        ...
                        newMessage.message = newQuest.name;
                        newMessage.type = "newQuest";
                        newMessage.save();

                        //Emit the events!
                        eventEmitter.emit('newnotification', user, newMessage);
                        eventEmitter.emit('newquestavailable', user, newQuest);
    ...
    module.exports.getUsername = function(req, res) {
        User.findOne({doc:req.params.doc})
            .then(function(user){
                ...
                else throw new customErr.notFoundError('User Not Found', 'User Not Found', 401);
            })
            .catch(function (err) { errorHandler.handleError(err, req, res) });
    };
    ...
    module.exports.resetPassword = function(req, res) {
        utils.hashPasswordAsync(req.body.password)
            .then(function(hash) {
                return User.findOneAndUpdate({doc:req.params.doc},{"credentials.password":hash});
            })
            .then( function (results){ utils.handleSuccess(messages.success.onAction("updating user"), res);})
            .catch(function (err) { errorHandler.handleError(err, req, res) });
    };
    ...
    module.exports.resetUsername = function(req, res) {
        User.findOneAndUpdate({doc:req.params.doc},{"credentials.username":req.body.username})
            .then( function (results){ utils.handleSuccess(messages.success.onAction("updating user"), res);})
            .catch(function (err) { errorHandler.handleError(err, req, res) });
    };
    ...
    module.exports.studentSpecificParentNewsletter = function(req, res){
        User.findOne({doc:req.body.doc})
            .populate('customer branches student.character.achievements.achievement')
            .then(function(st){
                statusMailsController.createStudentStatusMail(st, null, true);
