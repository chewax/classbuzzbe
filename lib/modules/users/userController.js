(function () {
    ...
    module.exports.activity = {
        add : function(req, res) {
            ...
            User.findOne({doc: req.params.doc})
                .then (function (user) {
                    // Keep working user and find and working activity
                    _user = user;
                    return activity.findOne({_id: req.body.activityId});
                })
                .then (function (activity) {
                    // Keep working activity and add activity to user
                    _activity = activity;
                    return _user.addActivity(activity);
                })
                .then (function (results){
                    // Create Associated event
                    _results = results;
                    eventEmitter.emit('activityaward', _user, _activity);

                    if (_activity.type == 'reward') {
                        var messageText = _user.firstName + ' ' + _user.lastName + ' has earned a "' + _activity.name + '" Award.';

                        //New requiremente, grant some gold whenever you receive an activity.
                        _user.student.currency.doubloons += Math.round(_activity.xpAward * 0.3 / 5) * 5;
                        _user.save();
                        return eventController.create(_user._id, 'activity Unlocked', messageText, _activity.xpAward, _activity._id, req.user.id);
                    }

                    if (_activity.type == 'penalty') {
                        var messageText = _user.firstName + ' ' + _user.lastName + ' has received a "' + _activity.name + '" Penalty.';
                        return eventController.create(_user._id, 'Penalty', messageText, _activity.xpAward, _activity._id, req.user.id);
                    }
        ...
        remove: function(req, res) {
            ...
            Event.findOneAndRemove({activity: req.body.activityId})
                .then( function (result) { return User.findOne( {doc: req.params.doc}) })
                .then (function (user) {
                    eventEmitter.emit('activityforfeit', _user, _activity);
                    return user.removeActivity (req.body.activityId);
                })
        ...
        all: function (req, res) {
            User.findOne( {doc: req.params.doc}, "student.character.activities")
                .populate('student.character.activities.activity')
                .then(function (results){ res.status(200).json(results.student.character.activities) ;})
                .catch(function (err) { errorHandler.handleError(err, req, res) });
        },
        ...
        find: function (req, res) {
            User.findOne( {doc: req.params.doc}, "student.character.activities")
                .populate('student.character.activities.activity')
                .then(function (results){
                    var activities = results.student.character.activities;
                    var sortedactivities = _.sortBy(activities, function(a){
                        return a.timestamp;
                    })
                    var toReturn = _.last(sortedactivities, req.body.limit);
    ...
    module.exports.branches = {
        ...
        update: function (req, res) {
            User.findOne( {doc: req.params.doc})
                ...
                .then( function (results){ utils.handleSuccess(messages.success.onAction("deleting activity"), res);})
                .catch(function (err) { errorHandler.handleError(err, req, res) });
        },
    ...
    module.exports.quests = {
        ...
        allWithactivity: function(req, res){
            User.findOne({doc: req.params.doc})
                .deepPopulate('student.quests.trigger.activity')
                .then( function (user){ res.status(200).json(user.student.quests); })
                .catch(function (err) { errorHandler.handleError(err, req, res) })
        },
    };
