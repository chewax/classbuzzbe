(function () {
     ...
     * Name: isFulfiled.
     */
    questGoal.virtual('isFulfiled').get(function(){
     ...
     */
    var characterQuests = new mongoose.Schema({
     ...
     * Name: newQuest.
     */
    characterQuests.virtual('newQuest').get(function(){
     ...
     * Name: progress.
     */
    characterQuests.virtual('progress').get(function(){
        var _totalSteps = 0;
     ...
     * Name: newQuest.
     */
    characterQuests.virtual('isCompleted').get(function(){
        //If it has a completed date set, then is completed.
        if (this.completed != null) return true;

        //Else loop through goals to see if they are fullfilled
        var _isCompleted = false;
        this.goals.forEach(function(g){
            _isCompleted = _isCompleted && g.isFulfiled;
        });

        return _isCompleted;
    });

    characterQuests.set('toObject', { virtuals: true });
    characterQuests.set('toJSON', { virtuals: true });
