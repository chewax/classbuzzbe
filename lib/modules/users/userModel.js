(function () {
    ...
    var userSettings = require('./schemas/userSettings');
    ...
    /**
     * Virtual Attribute to get parent children
     */
    userSchema.virtual('parent.children', {
        ref: 'User',
        localField: 'doc',
        foreignField: 'student.parents.doc'
    });
    ...
    /**
     * Returns an array of GroupLevel
     */
    userSchema.methods.userGroupLevel = function(options) {
        ...
        return Group.find({students: this._id})
            ...
            .then(function(groups){
                var groupLevels = groups.map(function(g){ return g.level });
                groupLevels = _.uniqBy(groupLevels, '_id');

                if (options.toString) return _.map(groupLevels, 'code' ).join(', ');
                else return groupLevels;
            });
    };
    ...
    userSchema.statics.findTeacherStudents = function(teacherId) {
        ...
        return new Promise (function(resolve, reject){
            ...
            var popQuery = [
                {
                    path: "students",
                    select: "firstName lastName email student.parents student.character student.house"
            ...
            Group.find({teacher:teacherId})
                ...
                .then(function(groups){
                    ...
                    groups.forEach(function(g){
                        ...
                        if (!_.isNil(g.students)) {
                            var _current = g.students.map( function(s){ s.level = g.level; return s });
                            _students = _students.concat( _current );
                        }
                    });
    ...
    var options = {
        populate: {
            ...
            'parent.children': {select: 'firstName lastName avatarURL birthday'},
            'parent.children.groupsBelonging': {select: 'name level teacher'},
            'parent.children.groupsBelonging.teacher': {select: 'firstName lastName phoneNumber'},

            'student.character.specialEvents.specialEvent.eligibleChests': {select: 'name xp money loot'},
            'student.character.specialEvents.specialEvent.skills': {select: 'name icon color customer'},
