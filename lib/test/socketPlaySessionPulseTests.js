 ...
 * @returns {*}
 */
function connectClient(user, session) {
...
describe("Socket IO Play Session Pulse Server",function(){
    ...
    it('Should rebound if answer is incorrect', function(done){
        ...
        client3.on('connect', function(data) {
            ...
            client3.emit( 'tieandjoin', user3.id, user3.customer );
            client3.emit( 'session.create', user3.id, 'play.pulse' );
            ...
            client3.on( 'session.game.start', function (_session) {
                client2.emit( 'session.event', _session._id, user2.id, 'game.pulse' );
                client1.emit( 'session.event', _session._id, user1.id, 'game.pulse' );
                client3.emit( 'session.event', _session._id, user3.id, 'game.pulse' );
            });

            client3.on( 'session.game.pulse', function (_session, user) {

                user._id.should.equal(user2.id);
                _session.status.should.equal('pulsed');
                _session.questionPoints.should.equal(100);
                client3.emit( 'session.event', _session._id, user3.id, 'game.pulse.evaluate', {correct:false} );
            });


            client3.on( 'session.game.rebound', function (_session, user) {
            ...
            client3.on( 'session.created', function ( _session ) {
                ...
                client1.on( 'connect', function ( data ) {
                    ...
                    client1.emit( 'tieandjoin', user1.id, user1.customer );
                    client1.emit( 'session.join', _session._id, user1.id );

                    client1.on( 'session.user.joined', function ( _session ) {
