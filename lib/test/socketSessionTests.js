...
describe("Socket IO Session Server",function(){
    ...
    it('Should NOT allow to JOIN a 2 different sessions', function(done){

        var client1, client2, client3;

        this.timeout(10000);

        var client3Connected = false;
        var client3Session = {};

        var client2Connected = false;
        var client2Session = {};

        client3 = _clientConnect(user3, 'play.pulse');
        client3.on('session.created', function(_session){
            client3Connected = true;
            client3Session = _session;

            if (client2Connected) goForIt();
        });

        client2 = _clientConnect(user2, 'play.pulse');
        client2.on('session.created', function(_session){
            client2Connected = true;
            client2Session = _session;

            if (client3Connected) goForIt();
        });


        function goForIt(){
            client1 = _clientConnect(user1, client2Session);

            client1.on('session.user.joined', function(_session, _user){
                client1 = _clientConnect(user1, client3Session);

                client1.on('session.error', function(_session, error){
                    error.code.should.equal(503);
                    should.equal(_session, null);
